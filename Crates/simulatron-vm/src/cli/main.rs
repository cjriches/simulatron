...
fn cli() -> Command {
    ...
    clap::command!()
        ...
        .arg(
            Arg::new(LOG_LEVEL)
                .help(
                    "Set the log level. Has no effect without \
        ...
        .arg(
            Arg::new(INIT)
                .help(
                    "Instead of running the VM, create a new skeleton directory \
    ...
    fn _run(args: ArgMatches) -> Result<(), String> {
        ...
        if args.get_flag(INIT) {
            return match create_skeleton() {
                ...
                Err(e) => Err(format!("Failed to create simulatron skeleton: {}", e)),
            };

            fn create_skeleton() -> io::Result<()> {
                fs::create_dir_all("./simulatron/DiskA")?;
                fs::create_dir_all("./simulatron/DiskB")?;
                File::create("./simulatron/ROM")
                    .and_then(|mut f| f.write_all(&[0; simulatron_vm::ROM_SIZE]))
            }
        }
        ...
        let rom = match fs::read(path) {
            Ok(rom) => rom,
            Err(e) => return Err(format!("Failed to open ROM file: {}", e)),
        };

        // Ensure the ROM is the correct size.
        if rom.len() != simulatron_vm::ROM_SIZE {
            return Err(format!(
                "ROM files must be exactly {} bytes in size.",
        ...
        // Run the Simulatron.
        simulatron_vm::run(rom.as_slice().try_into().unwrap(), disk_a_path, disk_b_path);
